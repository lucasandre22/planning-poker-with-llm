...
def get__story_points_average_from_tickets(tickets):
    context = 0
...
def get_response_based_on_ticket(ticket) -> dict:
    ...
    average = get__story_points_average_from_tickets(tickets)
...
def get_response_from_question(question) -> dict:
    result = QUESTION_ANSWERING_CHAIN.run(question=question)
...
if __name__ == "__main__":
    ...
    result = CHAIN_SIMILARITY.run(ticket=ticket, similar_tickets=similar_tickets)
    #print("\nTotal time spent by the model: ", time.time() - before)
    average = get__story_points_average_from_tickets(tickets)
